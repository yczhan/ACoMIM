...
def load_checkpoint(model,
    ...
    if state_dict.get("rel_pos_bias.relative_position_bias_table") is not None:
    ...
    if 'pos_embed' not in state_dict:
        print('we create sin-cos pos_embed now, num_patches 196, embed_dim 768')
        pos_emb = get_sinusoid_encoding_table(196, 768)
        cls_emb = torch.zeros([1, 1, 768], dtype=torch.float)
        state_dict['pos_embed'] = torch.cat((cls_emb, pos_emb), dim=1)
