...
# save_checkpoint() from MoBY
def save_model_moby(config, epoch, model, max_accuracy, optimizer, lr_scheduler, logger):
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            if 'optimizer' in checkpoint and 'epoch' in checkpoint:
                optimizer.load_state_dict(checkpoint['optimizer'])
                if isinstance(checkpoint['epoch'], str):
                    checkpoint['epoch'] = int(args.resume.split('.pth')[0].split('-')[-1])
                args.start_epoch = checkpoint['epoch'] + 1
...
def create_ds_config(args):
    ...
    with open(args.deepspeed_config, mode="w") as writer:
        ds_config = {
            "train_batch_size": args.data.batch_size * args.update_freq * get_world_size(),
            "train_micro_batch_size_per_gpu": args.data.batch_size,
